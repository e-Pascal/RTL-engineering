...
<details>
  ...
  <summary>Код реализации лабиринта</summary>
...
Пример смены цвета лабиринта с помощью переключателей:

![change color](pic/color2.gif)

<details>

  <summary>Код реализации движения игрока</summary>
...
<details>
  ...
  <summary>Код реализации делителя частоты движения персонажей</summary>
...
<details>
  ...
  <summary>Код реализации движения противника</summary>
...
Данная логика реализует движение противника посредством конечного автомата с несколькоми стадиями. Движение производится по пикселям, с каждым повтором противник становится все ниже, пока его вертикальная координата не превысит `128*3+2`, после чего он отправится наверх.

Движения противника:
- Спуск вниз на 128 пикселей;
- Движение влево на 256 пикселей;
- Ожидание 1 секунду;
- Движение вправо на 256 пикселей;

<details>

  <summary>Код реализации коллизий</summary>
...
you_coin        <= 0;
    ...
    stick_active      <= (col >= stick_border_hl_c) & (col <= (stick_border_hl_c + 
                      stick_width)) &  (row >= stick_border_hl_r) & (row <= 
                      (stick_border_hl_r + stick_height)) && (rom_data[11:0] != 12'hFFF);

    enemy_1_active 	  <= (col >= enemy_1_border_hl_c) & (col <= (enemy_1_border_hl_c + 
                         enemy_1_width)) & (row >= enemy_1_border_hl_r) & (row <= 
                         (enemy_1_border_hl_r + enemy_1_height))& (rom_ghost_data[11:0] != 12'hFFF);
...
Реализация коллизий сводится к проверке совпаданий координат 4-х сторон игрока с координатами сторон врага или монеты. Координаты высчитываются путем сложения ширины и высоты фигуры от точки отрисовки этой фигуры (левый верхний угол). При совпадении координат фигур поднимается флаг `you_lose` - означающий столкновение с противником - а значит проигрыш, либо `you_coin` - что означает подбор монеты. Следующая монета появляется на текущих координатах противника, что усложняет ее доступ. Флаги `stick_active`, `enemy_1_active` и `coin` обозначают границы отрисовки фигур игрока, противника и монеты соответственно, это необходимо для отрисовки их изображений, которая разобрана ниже.

<details>

  <summary>Код реализации отрисовки цветов</summary>
...
Отрисовка цветов реализована с помощью тернарного оператора, который поочередно сравнивает условия поднятых флагов, разобранных выше. Первым в сравнении является флаг `you_lose`, который обозначает проигрыш, при его активации красному цвету присваивается четыре бита единицы, а к другим цветам - нули, что будет отображаться как сплошной красный цвет, являющийся признаком проигрыша. Следующим в сравнении - флаг `coin`, если координата отрисовки находится в его границах - пикселю присваивается желтый цвет (красный + зеленый). Флаг `enemy_1_active`, обозначающий противника при активном состоянии, присваивает цвет, находящийся в ROM, проинициализированной `.mem` файлом, а он в свою очередь получается при конвертации изображения `64х64` пикселя с помощью [скрипта](png2mem/conv2mem.py). Флаг `stick_active` работает по такому же принципу, а если все предыдущие условия не выполняются, отрисовывается фон, то есть лабиринт игры.

## Описание работы с ROM

В данной реализации предусмотрена отрисовка игрока и призрака с разрешением только 64 на 64 пикселя, это можно достичь, скачав любую, желательно квадратную картинку с расширением `.png`, открыть ее в паинте и воспользоваться функцией сжатия, выбрав режим по пикселям, после чего сохранить с расширением `.png` при необходимости. Следующим шагом необходимо перенести получившуюся картинку в папку со [скриптом](png2mem). Воспользоваться им можно в терминале в папке с ним и полученной картинкой командой:

```Bash
python conv2mem.py ghost.png
```

В результате появится файл с расширением `.mem`, который необходимо скопировать в папку проекта и добавить в него, выбрав тип файла .`mem`.

![add mem](pic/mif_mem.png)

После чего в ваш проект будет добавлен этот файл, которым можно проинициализировать ROM.
